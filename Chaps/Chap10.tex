...
\Next
若$\chi_i$和$\chi_j$是某单电子算符$h$(见\ref{2.29})的本征函数, 
对应本征值为$\epsilon_i,\epsilon_j$. 
证明, 
方程\ref{2.33a} \ref{2.33b}中的Hartree积和\ref{2.34}中的反对称波函数都是独立电子哈密顿量$\scr{H}=h(1)+h(2)$(见\ref{2.28})的本征函数, 
...
电子2占据$\psi_2$, 
那么\ref{2.45}中第一项是在$\db{r_1}$($\db{r_1}$附近)找到电子1的概率乘以在$\db{r_2}$($\db{r_2}$附近)找到电子2的概率. 
...
\end{figure}
之前提到(\ref{2.60})该模型有四($2K=4$)个自旋轨道$\chi_1,\chi_2,\chi_3,\chi_4$. 
$N=2$, 
...
因此这些态对称性都是u(正乘负得负). 
\underline{极小基$\mathrm{H}_2$}的精确波函数$\ket{\Phi_0}$的对称性与Hartree-Fock近似下的基态一样, 
都有$g$对称性. 
因此, 
...
然后再约化为仅包含空间轨道$\psi_i$的积分. 
 我们将以\underline{极小基$\mathrm{H}_2$}为例展示该过程, 
然后给出求这种矩阵元的一般规则.


\subsection{\underline{极小基$\mathrm{H}_2$}的矩阵元}
 \label{sec2.3.1}
我们先来计算\underline{极小基$\mathrm{H}_2$}模型的full CI矩阵元(方程\ref{2.79}). 
...
表\ref{t2.3}、\ref{t2.4}总结了这三种情况下的规则。
注意，
两个行列式的差异越大，
矩阵元的形式就越简单，
即其中的项数越少。
...
这个事实可以帮我们记忆如何把单个行列式对应的能量用关于自旋轨道的单、双电子积分表示出来：\textit{ 每个占据的自旋轨道$\chi_i$各在总能中贡献一个$\braket{i|h|i}$项，并且每一对占据自旋轨道$\chi_i,\chi_j$都贡献一个$\braket{ij||ij}$项。}因此，
...
\exercise{
将练习2.
14的结果推广到$N$电子行列式上： 已知 由自旋轨道组成的Slater行列式$\ket{\chi_i\chi_j\cdots\chi_k}$是单电子算符$h$(见式2.
29)的本征函数, 
请证明它也是单电子哈密顿$\mathscr{H}=\sum_{i=1}^{N}h(i)$(见式2.
28)的本征函数, 
...
否则就总会有一个$0$因子出现(因为$\ket{L}$中的某自旋轨道如$\chi_n'$与$\ket{K}$中的自旋轨道$\chi_m,\chi_n,\cdots$正交). 
所以, 
...
\end{align}
与上一情形中一样, 
可以通过交换\underline{积分傀标}证明上式中的两项是相等的. 
所以有
\begin{align}
...
\end{equation}
带横线的轨道的其他排列方式积分后都为0. 
因此\underline{极小基$\mathrm{H}_2$}的Hartree-Fock基态能就是
\begin{align}
...
\exercise{
对自旋变量积分, 
证明\underline{极小基$\mathrm{H}_2$}的full CI矩阵可以写成
\begin{equation}
...
与\underline{极小基$\mathrm{H}_2$}的波函数
\begin{equation}
...
如此由如下结论: \emph{空间轨道$\psi_i$上的一个电子(无论自旋如何)会在总能量中贡献一项$h_{ii}$.}.
...
\end{align}
因此我们由如下论断：\emph{空间轨道$\psi_i,\psi_j$中的每一对电子(无论其自旋)都对能量贡献一项$J_{ij}$. 空间轨道$\psi_i,\psi_j$中的一对自旋平行电子对能量贡献一项$-K_{ij}$}. 
行列式对应的总能就是这些项之和.
...
\exercise{
令$\Psi_0 = \ket{\chi_1\chi_2} = \cs_1\cs_2\ket{\,\,}$为\underline{极小基$\mathrm{H}_2$}的Hartree-Fock波函数. 
请用二次量子化的办法证明
\begin{align*}
...
可见练习2.
38. 
最简单的闭壳层行列式就是\underline{极小基$\mathrm{H}_2$}的Hartree-Fock基态波函数：
\begin{align}
...
12中那样. 
举个例子, 
考虑\underline{极小基$\hd$}的四个单激发行列式(见式(2.
76), 
四个都是开壳层行列式). 
其中的两个
\begin{subequations}
...
下面将刚才所得的\underline{极小基$\hd$}的结果推广. 
第四章和第五章会用到单重自旋匹配组态, 
...
\end{align}
若$a=1,b=2$则上面的式子就退化为\underline{极小基$\hd$}的情形, 
即(2.
260).
