...
\end{tabular}
\end{center}
请用附录~\autoref{appendix:a}中的重叠积分表达式导出He 4-31G的收缩参数。
}
...
分子$\hd$只有一个占据轨道。在各种基组下，该占据轨道的能量负值如表\autoref{tab:3.14}所示，所有$\hd$轨道如图\autoref{fig:3.9}所示。超过极小基的那些基组下，占据轨道能量有微小变化，但对所有超过极小基的那些基组，轨道能量都保持固定在 -0.595 Hartrees。 预测出的电离势（+0.595 Hartrees）误差仅有2\%。在表\autoref{tab:3.14}以及随后的表格中，所有电离势都是垂直电离势而不是绝热电离势。垂直跃迁种，最终状态具与初始状态核几何结构相同；绝热跃迁种，跃迁前后核都在平衡几何结构上。Koopmans电离势的值与实验值之间的出色一致性是由于电离势中的关联效应和弛豫效应恰好相互抵消了，而两种效应在Koopmans近似里实际上被忽略了。电子关联对终态的单电子物种$\hd^+$没有影响，但它降低了初态的$\hd$的能量。弛豫效应则是降低了终态的$\hd^+$的能量。这两种效应在$\hd$这个例子种几乎相互抵消了。

表\autoref{tab:3.15}列出了$\mathrm{CO}$的两个Koopmans电离势。$\mathrm{CO}$的最好占据分子轨道是成键轨道$5\sigma$和$1\pi$。这两个轨道主要
...
\end{align}
\autoref{3.312}\autoref{3.313}这两个本征值方程。
\subsection{非限制性密度矩阵}
\subsection{Fock矩阵的表达式}
