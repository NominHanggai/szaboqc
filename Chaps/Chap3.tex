...
式\eqref{3.219}到(3.221)中将STO-LG拟合到Slater函数时Slater指数为$\zeta=1.0$. 
...
\exercise{
	用STO-1G的定义式\eqref{3.219}和比例关系(3.224)证明, 
...
\exercise{对$\psi_1,\psi_2$进行归一化, 由此导出$\psi_1,\psi_2$的基函数展开中的系数$[2(1+S_{12})]^{-1/2}$及$[2(1-S_{12})]^{-1/2}$.
	...
	用以上所得系数求解式\eqref{3.234}, 得到\phrase{极小基 $\hd^+$}在$R=1.4\au$的轨道能量, 验证它们是
	\begin{align*}
	...
	\end{align*}
	\Next
	用总电子能量的一般式\eqref{3.184}证明\phrase{极小基 $\hd$}的电子能量为
	\begin{align*}
...
\ref{t3.3}列出了不同$1s$ Slater轨道指数$\zeta$下的\phrase{极小基 $\hd$}($R=1.4$)总能量. 
...
\end{tabular}
\end{center}
请用附录~\ref{appendix:a}中的重叠积分表达式导出He 4-31G的收缩参数。
}
...
表\ref{tab:3.11} 至 \ref{tab:3.13}展示了使用四个基组
STO-3G、4-31G、6-31G* 和 6-31G** 算出的表\ref{tabe:3.10}中分子的总能量。
 $\hd$没有内壳层，也不是重原子，
...
\exercise{
利用表 \ref{tab:3.11} 到\ref{tab:3.13}的结果，计算每个基组下，下面两个反应的能量差，再计算Hartree-Fock极限下的能量差：
\begin{align*}
...
分子$\hd$只有一个占据轨道。在各种基组下，该占据轨道的能量负值如表\ref{tab:3.14}所示，所有$\hd$轨道如图\ref{fig:3.9}所示。超过极小基的那些基组下，占据轨道能量有微小变化，但对所有超过极小基的那些基组，轨道能量都保持固定在 -0.595 Hartrees。 预测出的电离势（+0.595 Hartrees）误差仅有2\%。在表\ref{tab:3.14}以及随后的表格中，所有电离势都是垂直电离势而不是绝热电离势。垂直跃迁种，最终状态具与初始状态核几何结构相同；绝热跃迁种，跃迁前后核都在平衡几何结构上。Koopmans电离势的值与实验值之间的出色一致性是由于电离势中的关联效应和弛豫效应恰好相互抵消了，而两种效应在Koopmans近似里实际上被忽略了。电子关联对终态的单电子物种$\hd^+$没有影响，但它降低了初态的$\hd$的能量。弛豫效应则是降低了终态的$\hd^+$的能量。这两种效应在$\hd$这个例子种几乎相互抵消了。

表\ref{tab:3.15}列出了$\mathrm{CO}$的两个Koopmans电离势。$\mathrm{CO}$的最好占据分子轨道是成键轨道$5\sigma$和$1\pi$。这两个轨道主要
...
它们对应的两个\phrase{积分-微分-本征值方程}\eqref{3.312}\eqref{3.313}无法相互独立地求解。
也即，
