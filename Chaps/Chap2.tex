...
\subsection{矩阵元规则的推导}
\label{sec2.3.4}
本节我们来推导\autoref{t2.3}和\autoref{t2.4}中的矩阵元的计算规则. 
这些矩阵元就是单、双电子算符夹在$N$电子行列式之间的算式($N$电子行列式由自旋轨道构成). 
由自旋轨道$\chi_i(\mathbf{x}_1),\chi_j(\mathbf{x}_1),\cdots,\chi_k(\mathbf{x}_N)$构成的$N$电子行列式就是(见\autoref{eq:1.38})):
...
\end{align}
与上一情形中一样, 
可以通过交换\phrase{积分变量}证明上式中的两项是相等的. 
所以有
\begin{align}
...
下面考虑入\autoref{2.157}基态能量表达式中的第一项双电子积分
\begin{align}
