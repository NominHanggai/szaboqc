...
当一个并矢与一个矢量进行点乘操作时，
会得到一个新的矢量。
而单位并矢与矢量点乘则会得到原来的矢量。公式\eqref{eq:1.10}被称作基$\{\vec{e}_i\}$ 的\emph{完备性关系}。
...
\begin{equation}
 ...
 \label{eq:1.14}
\end{equation}
这里，$\mbf{O}$被称作算符$\op{O}$在基$\bs{e}{i}$的\emph{矩阵表示}。
...
\exercise{
 ...
 \begin{enumerate}[1.]
    ...
    \item 若$\mbf B$为幺正，且有关系${\mbf B} = \madj{U}{\mbf A}{\mbf U}$，则有${\mbf A} = {\mbf U}{\mbf B}{\madj U}$。
    \item 若两个Hermitian矩阵${\mbf A}$和${\mbf B}$的乘积${\mbf C} = \mbf{AB}$也是Hermitian矩阵，则${\mbf A}$和${\mbf B}$对易。
    \item 若${\mbf A}$为Hermitian矩阵，则它的逆${\mbf A}^{-1}$，如果存在，同为Hermitian矩阵。
    \item 如果
        \[ {\mbf A} = 
        \begin{pmatrix}
...
\Next
 对照方程\eqref{eq:1.39}，
 我们发现在练习 \ref{ex:1.4}f 中得到的$2\times 2$矩阵 $\mbf A$的逆可以写作：
...
\begin{equation}
 ...
 \label{eq:1.42}
\end{equation}
这里 $\mbf a$是抽象的矢量 $\ket{a}$在基 $\left\{\ket{i}\right\}$ 中的矩阵表示。
方程\eqref{eq:1.27}提到，
...
怎样才能得到它在基$\kbs{i}$或$\bbs{i}$中的各个分量呢？
...
\begin{equation}
 ...
 \label{eq:1.49}
\end{equation}
利用这个结果可以将方程\eqref{eq:1.41}和\eqref{eq:1.46}写成如下形式：
\begin{subequations}
 \begin{equation}
...
\begin{equation}
 ...
 \label{eq:1.53}
\end{equation}
这里$\mbf O$是算符$\op{O}$在基$\kbs{i}$内的矩阵表示。
...
我们知道了基底的选择不是唯一的。
给定两个完备的正交归一基$\kbs{i}$和$\kbs{\alpha}$，
本小节中我们将找出他们之间的关系。
我们把第一个基下左矢和右矢用拉丁字母$i, j, k, \dots$标记；
第二个基下的矢量用希腊字母$\alpha, \beta, \gamma, \dots$标记。
这样，我们有：
\begin{subequations}
 \begin{equation}
...
\end{subequations}
由于基底$\kbs{i}$是完备的，
我们可以将基$\kbs{\alpha}$中的任一基矢$\ket{\alpha}$展开为基底$\kbs{i}$中基矢的线性组合，
反之亦然。即：
\begin{equation}
...
可以通过该矩阵由一个基底变换至另一基底，
如式\eqref{eq:1.63}，
或者通过式\eqref{eq:1.65}变换回至原基底；
这个幺正变换矩阵的矩阵元即为两个基底中基矢的标量积，
可通过式\eqref{eq:1.64}计算。

下面我们考虑在不同的基底下，
算符 $\op{O}$ 的矩阵表示具有什么样的联系。
下一步我们得到的结果将在下一小节关于本征值问题的讨论中占据着中心地位。
假设$\mbf O$是算符$\op{O}$在基$\kbs{i}$中的矩阵表示，
而$\boldsymbol{\Omega}$是该算符在基$\kbs{\alpha}$中的矩阵表示，
则有：
\begin{subequations}
 \begin{equation}
...
通常情况下，
Hermitian算符在一个任意基$\kbs{i}$下的矩阵表示并不是对角化的，
但它在其本征矢构成的基底内的矩阵表示是对角的。
...
关于本征值问题，我们可以这样理解：
给定一个 Hermitian算符$\op{O}$在一个正交归一基$\left\{\ket{i},\; i = 1, 2, \dots, N\right\}$ 的矩阵表示$\mbf O$，
我们需要找到一个新的基底 $\left\{\ket{\alpha},\; \alpha = 1, 2, \dots, N\right\}$，
并要求在此基底中，
...
对角化 Hermitian矩阵有种类繁多的高效算法\endnote{
如 J. H. Wilkinson. 
...
\exercise{
 对于所有的$\alpha = 1, 2, \dots, N$，证明方程\eqref{eq:1.90}中包含方程\eqref{eq:1.87}。
}
...
方程\eqref{eq:1.94}有非平凡解得条件是其久期行列式值为零：
\begin{equation}
 \begin{vmatrix*}[l]
...
\begin{subequations}
 ...
 \label{eq:1.99}
\end{subequations}
为了求解对应于$\omega_2$的本征矢，将$\omega_2$带入方程\eqref{eq:1.97a}，我们有：
...
\exercise{
 ...
 这样方程\eqref{eq:1.94}变为
 \[O_{11} + O_{12}c = \omega\]
 \[O_{21} + O_{22}c = \omega c\]
 通过消元法求解所得方程，
 并验证结果与式\eqref{eq:1.96}相同。
 这种方法可以用于求解矩阵的最低本征值，
 是一种不需要行列式的久期行列式方法，
 将在本书中多次用到。利用这种方法，
...
\begin{subequations}
 ...
 \label{eq:1.106}
\end{subequations}
与方程\eqref{eq:1.104}和\eqref{eq:1.89}比较，我们得到两个本征矢：
\begin{subequations}
 \begin{equation}
     \begin{pmatrix}
      c_1^1 \\ c_2^1 
...
\begin{equation}
 ...
 \label{eq:1.117}
\end{equation}
也就是说，基$\left\{\psi_i(x)\right\}$ 是完备的。
给定函数$a(x)$后，
...
直到现在我们的讨论都与~\ref{sec:1.1.4}中关于$N$维复矢量空间的讨论类似。
...
\exercise{
 ...
 考虑算符$\op{O}$在基$\left\{\psi_i(x)\right\}$中的矩阵表示：
 已知
 \[\op{O}\psi_i(x) = \sum_j\psi_j(x)O_{ji}\]
 证明
 \[O_{ji} = \int\mrm{d}x\,\psi_j^\ast(x)\op{O}\psi_i(x)\]
 然后应用方程\eqref{eq:1.127a}和\eqref{eq:1.138}，
 用 Dirac 符号重写第一个方程，
 并证明其与方程\eqref{eq:1.55}等价。
\Next
 考虑如下本征值问题：
 \[\op{O}\phi(x) = \omega\phi(x)\]
 通过将$\phi$在完备基$\left\{\psi_i(x),\, i = 1, 2, \dots\right\}$ 中展开：
 ...
 而实际操作中，我们无法处理无穷矩阵。
 为了使问题可操作，
 我们可以选取集合$\{\psi_i(x)\}$的一个子集$\left\{\psi_i(x),\, i = 1, 2, \dots, N\right\}$。
 如果在这个子空间中重复以上分析，
 我们会得到一个$N\times N$维的本征值问题。
 我们将在第~\ref{sec:1.3} 节中看到，
...
\]
引入连续变量的无穷个基矢的完全集，
其基矢记为$\ket{x}$。与式\eqref{eq:e1.17.1a}类似：
\[
...
\Next
 ...
 即$\theta_0$。
 验证$\omega(\theta_0)$与矩阵$\mbf O$的最低本征值（见式~\ref{eq:1.105} 和式~\ref{eq:1.106a}相等。
 你对这个结果有什么想法？
}
...
Hamiltonian 算符在基$\left\{\vert\Psi_i\rangle\right\}$中的矩阵表示$\mbf{H}$是一个$N\times N$的矩阵，
其矩阵元为：
\begin{equation}
...
\exercise{
 ...
 \begin{enumerate}[a.]
     \item 推广第~\ref{sec:1.3.1}~小节中关于变分原理的证明，并验证
     ...
     其中$\ket{\tilde\Phi_\alpha}, \alpha = 0, 1$由方程\eqref{eq:1.168}给定。
     若该函数归一，证明
...
\end{equation}
寻找最优近似解的一种方法。
该方法需要在有限基$\left\{\ket{\Psi_i}\right\}$内构建算符$\op{H}$的矩阵表示，
...
如果使用\emph{完备}的正交归一基 $\left\{\ket{\Psi_i},
...
\begin{description}
 ...
 \item{Press, W. H., Flannery, B. P., Teukolsky, S. A. and Vetterling, W. T. \textit{Numerical Recipes}, Cambridge University Press, Cambridge, 1986.} 在这部科学计算的圣经中，第11章讨论了求解矩阵本征值和本征矢的多种算法，并提供了 FORTRAN 源码。
\end{description}