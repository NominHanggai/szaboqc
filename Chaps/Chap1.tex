...
通过求算\renewcommand*\equationautorefname{公式}\autoref{eq:1.1}\renewcommand*\equationautorefname{式}与基矢 $\vec{e}_j$ 之间的标量积来得到它的三个分量：
\begin{equation}
 \vec{e}_j \cdot \vec{a} = \sum_i \vec{e}_j \cdot \vec{e}_i\, a_i = \sum_i \delta_{ji} a_i = a_j
 \label{eq:1.8}
\end{equation}
这样我们又可以将\renewcommand*\equationautorefname{公式}\autoref{eq:1.1}\renewcommand*\equationautorefname{式}写成如下形式：
\begin{equation}
...
当一个并矢与一个矢量进行点乘操作时，
会得到一个新的矢量。
而单位并矢与矢量点乘则会得到原来的矢量。\renewcommand*\equationautorefname{公式}\autoref{eq:1.10}\renewcommand*\equationautorefname{式}被称作基组 $\{\vec{e}_i\}$ 的\emph{完备性关系}。
它其实是\renewcommand*\equationautorefname{公式}\autoref{eq:1.1}\renewcommand*\equationautorefname{式}的变形，
...
\begin{equation}
 ...
 \label{eq:1.17}
\end{equation}
这样，如果我们通过\renewcommand*\equationautorefname{公式}\autoref{eq:1.16}\renewcommand*\equationautorefname{式}定义矩阵的乘法，
...
与\renewcommand*\equationautorefname{公式}\autoref{eq:1.11}\renewcommand*\equationautorefname{式}类似，
我们通过作用在右矢上的效果来定义算符：
