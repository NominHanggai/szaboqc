...
Goldstone的\mci{连接簇}(linked-cluster)定理一般性地用图技术证明了Brueckner猜想， 即RS微扰论在每一阶都是尺寸一致(size-consistent)的。那些正比于$N^2$的项所对应的图包含分离的几部分（即\emph{未连接}图）.Goldstone用含时的技术证明了这类图一定不回出现在$n$阶能量的最终表达式中。因此Goldstone将$n$阶能量用\emph{连接}图写出来。本章内，我们仅运用了连接图。我们希望，通过这么多对一致性的检查，（比较用图和代数式所得的两种结果），能令读者相信RS微扰论的所有形式(即$N$-态、轨道等等形式)都能仅用连接图表示出来（或者至少相信这个说法是合理的）。需要强调，``尺寸一致"这个术语在本书中的意思是，在某个理论中，能量按照多体系统的尺寸线性增长。仅当系统是$N$个独立的闭壳层单个分子组成的超分子，而且所用的方法在简并轨道的酉变换下不变，尺寸一致这个词意味着系统的能量是$N$倍的单体能量。
\exercise{
