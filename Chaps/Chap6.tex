...
\begin{table}[H]
	...
	\begin{tabular}{ccc}
		...
		\coordinate (c) at ($(a)+(0,-2)$);
		\coordinate (d) at ($(c)+(2,0)$);
		\path[mid arrow seg=.45,draw=blue]
		(a) arc(135:225:1.414) node[midway,left]{$a$}  
		(c) arc(-45:0:1.414) coordinate(m) node[midway,right]{$r$} 
...
\exercise{
某个超分子由$N$个无相互作用的极小基$\hd$分子组成，请通过计算\ref{t6.2}中Goldstone图，得到该系统的$E_0^{(3)}$。将你的结果与\eqref{6.92}相比，后者是从代数上直接消掉正比于$N^2$的项后得到的。\emph{提示:} 直接证明这个超分子的每一个Goldtone图的值都是$N$乘以单个分子的对应值。
}
\section{一些算例}