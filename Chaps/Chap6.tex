...
尽管微扰理论给出的二阶能量极好地预测了包含氢的化学键的长度，但对于多重键来说并非如此。 表 \ref{tab:6.7} 给簇了 SCF 和微扰理论算得的 $\mathrm{N_2}$ 和 $\mathrm{CO}$ 键长，并与实验做了比较。 对于最大的基组，SCF 低估了实验结果约 0.03 a.u.； 二阶微扰理论预测的键长增大了，但它显著高估了校正。 三阶校正部分抵消了二阶校正的效果,但与实验的一致性仍不能完全令人满意。
...
\begin{table}[H]
	...
	\begin{tabular}{lccccccc}
		...
		\cline { 2 - 4 } \cline { 5 - 8 } 基组 & SCF & $E_0^{(2)}$ & $E_0^{(2)}+E_0^{(3)}$ & & $\mathrm{SCF}$ & $E_0^{(2)}$ & $E_0^{(2)}+E_0^{(3)}$ \\
