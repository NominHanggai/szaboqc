...
\section{轨道微扰论的图表示}
\ref{sec6.2}中我们为RS微扰论引入了一种完全普适的图表示. 为了利用它来处理轨道微扰, 我们用上线和下线来分别表示穴和粒子的自旋轨道, 用点来对应单粒子微扰$\v$. 然后我们画出和之前同样的一组图, 用指标$a$, $b$, ...来标记穴, 用$r$, $s$, ...来标记粒子. 所以就有
\begin{center}	
...
\sum^N = 2\sum^{N/2}
\end{align}
我们会在\ref{sec6.7}中看到，更一般的情形下，做这种转换时，要把每个图的值乘以$2^l$, $l$是闭合圈图的数目。
\exercise{
求出闭壳层闭壳层环多烯的四阶能量：
\begin{enumerate}[a.]
	\item 证明
