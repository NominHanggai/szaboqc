...
\begin{table}[H]
	...
	\begin{tabular}{ccc}
		...
		& $(-)^{2+2}\left(\frac{1}{2}\right)\sum_{abrs}\frac{\braket{ab|rs}\braket{rs|ab}}{\epsilon_{a}+\epsilon_{b}-\epsilon_{r}-\epsilon_{s}}$\\\hline		2 & \tikz[baseline={(current bounding box.center)},scale=.6]{
					\coordinate (e) at (4,-2.5+2);
					\coordinate (f) at ($(e)+(2,0)$);
					\coordinate (g) at ($(e)+(0,-2)$);
					\coordinate (h) at ($(g)+(2,0)$);
		...
		& $(-)^{4+2}\left(\frac{1}{2}\right)\sum_{abcdrs}\frac{\braket{ad|rs}\braket{cb|ad}\braket{rs|cb}}{(\epsilon_{a}+\epsilon_{c}-\epsilon_{r}-\epsilon_{s})(\epsilon_{c}+\epsilon_{b}-\epsilon_{r}-\epsilon_{s})}$\\\hline
					...
					3 & \begin{tikzpicture}[baseline={(current bounding box.center)},scale=.6]
		\coordinate (a) at (4,-2.5+2);
		\coordinate (b) at ($(a)+(2,0)$);
		\coordinate (c) at ($(a)+(0,-2)$);
		\coordinate (d) at ($(c)+(2,0)$);
		...
		\path[use as bounding box] ($(c)-(.5,.5)$) rectangle ($(b)+(.5,.5)$);
		\end{tikzpicture} 
		& $(-)^{3+2}\sum_{abcrst}\frac{\braket{ac|rt}\braket{rb|sc}\braket{st|ab}}{(\epsilon_{a}+\epsilon_{c}-\epsilon_{r}-\epsilon_{t})(\epsilon_{a}+\epsilon_{b}-\epsilon_{s}-\epsilon_{t})}$\\\hline
		
					4 & \begin{tikzpicture}[baseline={(current bounding box.center)},scale=.6]
		\coordinate (a) at (4,-2.5+2);
		\coordinate (b) at ($(a)+(2,0)$);
		\coordinate (c) at ($(a)+(0,-2)$);
		\coordinate (d) at ($(c)+(2,0)$);
		...
		\path[use as bounding box] ($(c)-(.5,.5)$) rectangle ($(b)+(.5,.5)$);
		\end{tikzpicture} 
		& $(-)^{3+2}\sum_{abcrst}\frac{\braket{bc|rt}\braket{ra|sb}\braket{st|ac}}{(\epsilon_{b}+\epsilon_{c}-\epsilon_{r}-\epsilon_{t})(\epsilon_{a}+\epsilon_{c}-\epsilon_{s}-\epsilon_{t})}$\\\hline
		
		5 & \begin{tikzpicture}[baseline={(current bounding box.center)},scale=.6]
		\coordinate (a) at (0,0);
		...
		\path[use as bounding box] ($(c)-(.5,.5)$) rectangle ($(b)+(.5,.5)$);
		\end{tikzpicture} 
		& $(-)^{3+3}\sum_{abcrst}\frac{\braket{ac|rt}\braket{bt|sc}\braket{rs|ab}}{(\epsilon_{a}+\epsilon_{c}-\epsilon_{r}-\epsilon_{t})(\epsilon_{a}+\epsilon_{b}-\epsilon_{s}-\epsilon_{t})}$\\\hline
	\end{tabular}
\end{table}
...
\begin{table}[H]
	...
	\begin{tabular}{ccc}\hline
		...
		\path[use as bounding box] ($(c)-(.5,.5)$) rectangle ($(b)+(.5,.5)$);
		\end{tikzpicture}
		& $(-)^{4+1}\left(\frac{1}{2}\right)\sum_{abcrst}\frac{\braket{cb|rt}\braket{ra|sb}\braket{st|ac}}{(\epsilon_{c}+\epsilon_{b}-\epsilon_{r}-\epsilon_{t})(\epsilon_{a}+\epsilon_{c}-\epsilon_{s}-\epsilon_{t})}$\\\hline
		...
		\path[use as bounding box] ($(c)-(.5,.5)$) rectangle ($(b)+(.5,.5)$);
		\end{tikzpicture} 
		& $(-)^{3+2}\sum_{abcrst}\frac{\braket{ab|rs}\braket{sc|at}\braket{rt|bc}}{(\epsilon_{a}+\epsilon_{c}-\epsilon_{r}-\epsilon_{s})(\epsilon_{c}+\epsilon_{b}-\epsilon_{s}-\epsilon_{t})}$\\\hline
		...
		\path[use as bounding box] ($(c)-(.5,.5)$) rectangle ($(b)+(.5,.5)$);
		\end{tikzpicture} 
		& $(-)^{3+2}\sum_{abcrst}\frac{\braket{bc|rt}\braket{at|sc}\braket{rs|ab}}{(\epsilon_{b}+\epsilon_{c}-\epsilon_{t}-\epsilon_{r})(\epsilon_{a}+\epsilon_{b}-\epsilon_{r}-\epsilon_{s})}$\\\hline
	\end{tabular}
\end{table}
...
\subsection{对图求和}
\ref{sec6.2.3}节中我们已经看到，可以某一类特定的图求和至无穷阶，因为
\subsection{什么是Linked-Cluster定理?}
Goldstone的\mci{连接簇}(linked-cluster)定理一般性地用图技术证明了Brueckner猜想， 即RS微扰论在每一阶都是尺寸一致(size-consistent)的。那些正比于$N^2$的项所对应的图包含分离的几部分（即\emph{未连接}图）.Goldstone用含时的技术证明了这类图一定不回出现在$n$阶能量的最终表达式中。因此Goldstone将$n$阶能量用\emph{连接}图写出来。本章内，我们仅运用了连接图。我们希望，通过这么多对一致性的检查，（比较用图和代数式所得的两种结果），能令读者相信RS微扰论的所有形式(即$N$-态微扰论、轨道微扰论等等形式)都能仅用连接图表示出来（或者至少相信这个说法合理）。需要强调，``尺寸一致"这个术语在本书中的意思是，在某个理论中，能量按照多体系统的尺寸线性增长。仅当系统是$N$个独立的闭壳层单个分子组成的超分子，而且所用的方法在简并轨道的酉变换下不变时，尺寸一致这个词才意味着系统的能量是$N$倍的单体能量。
\exercise{
某个超分子由$N$个无相互作用的极小基$\hd$分子组成，请通过计算\ref{t6.2}中的Goldstone图，得到该系统的$E_0^{(3)}$。将你的结果与\eqref{6.92}相比，后者是从代数上直接消掉正比于$N^2$的项后得到的。\emph{提示:} 直接证明这个超分子的每一个Goldtone图的值都是$N$乘以单个分子的对应值。
}
\section{一些算例}